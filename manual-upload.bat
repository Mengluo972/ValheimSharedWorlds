...
rem  用法：把本文件放到游戏根目录（或子目录）双击运行，按提示操作。
rem  前提：Windows 10/11 自带的 curl 与 PowerShell，无需安装任何东西。
rem  说明：Repo / Branch / PathPrefix 读取 BepInEx\config 下的模组 cfg，
rem        Token 读取 %AppData%\ValheimSaveShare\token.dat（与游戏内模组同一来源）；
rem        每次运行都重新读文件，改完立即生效，无需重启游戏。
rem ============================================================
...
goto findgame
:nogame
echo [错误] 没找到 valheim.exe，请确认本脚本放在 Valheim 游戏目录（或其子目录）里再运行。
pause
exit /b 1
:foundgame
...
rem ---- 从 cfg 读取配置（Token 不在这里，见下方 token.dat） ----
set "REPO="
set "BRANCH="
set "PREFIX=worlds"
...
rem ---- 从 token.dat 读取 Token（与游戏内模组同一来源） ----
rem 文件里含说明文字，只取第一条 github_pat_ / ghp_ 开头的行；Get-Content 读取时顺带剥掉 UTF-8 BOM。
...
if "%TOKEN%"=="" echo [警告] 读不到 Token（上传必需）: %TOKENFILE%
if "%TOKEN%"=="" echo         请把 github_pat_ 开头的 Token 粘贴到该文件（首次运行游戏会自动生成带教程的模板）。
if "%REPO%"=="" goto end
if "%TOKEN%"=="" goto end
